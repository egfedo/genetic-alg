...
class MatrixGetterFromGui:
    ...
    def __init__(self, gui):
        ...
        lbl = tk.Label(self.win, text="Количество вершин в графе: ", font='Calibri 14')
        entry = tk.Entry(self.win, bg="#fafafa", font='Calibri 14', width=3, relief="solid", bd=2, )
        btn_for_count_v = tk.Button(self.win, text="Построить матрицу для ввода", bg='silver', font='Calibri 14', relief="solid",
                                    bd=2,
        ...
        btn_for_save = tk.Button(self.win, text="Задать граф", bg='silver', font='Calibri 14', relief="solid", bd=2,
                                 command=self.save_matrix)
        btn_for_rand = tk.Button(self.win, text="Случайные веса", bg='silver', font='Calibri 14', relief="solid", bd=2,
    ...
    def create_new_matrix(self):
        ...
        lbl = tk.Label(self.frm_matrix, text='', bg='grey', font='Calibri 14', relief="solid", bd=2)
        lbl.grid(row=0, column=0, stick='wens')

        for i in range(1, self.count_v + 1):
            lbl = tk.Label(self.frm_matrix, text=str(i), bg='grey', font='Calibri 14', relief="solid", bd=2)
            lbl.grid(row=i, column=0, stick='wens')
            self.frm_matrix.columnconfigure(i, minsize=cell_size)

        for j in range(1, self.count_v + 1):
            lbl = tk.Label(self.frm_matrix, text=str(j), bg='grey', font='Calibri 14', relief="solid", bd=2)
        ...
        for i in range(self.count_v):
            lbl = tk.Label(self.frm_matrix, bg="gray", font='Calibri 14', width=2, relief="solid", bd=2,
                           justify='center', text='')
            lbl.grid(row=i + 1, column=i + 1, stick='wens')
            for j in range(i):
                entry = tk.Entry(self.frm_matrix, bg="#fafafa", font='Calibri 14', width=2, relief="solid", bd=2,
                                 justify='center')
